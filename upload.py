...
class Gifuploader(object):
    ...
    def uploadgif(self, img_path, move_path, filename, query_word, tag):
        ...
        try:
            ...
            upload_res =  self.session.post("https://flyingaway-backend-FlyingAway.app.secoder.net/api/gif/upload/", data=upload_item, headers = self.upload_header, timeout = 30)
    ...
    def fix_query_word(self, query_word):
        if (query_word in ["体育运动", "体育", "运动"] or query_word in self.tiyu_list):
            query_word = "体育运动"
        elif (query_word in ["动物世界", "动物", "猫", "狗", "鸭"] or query_word in self.dongwu_list):
            query_word = "动物世界"
        elif (query_word in ["影视片段", "影视", "电影", "电视剧"] or query_word in self.yinghsi_list):
            query_word = "影视片段"
        elif (query_word in ["动画表情", "动画表情包", "动画" , "卡通", "卡通表情包"]):
            query_word = "动画表情"
        elif (query_word in ["人物反应", "惊讶", "愤怒", "人物喜悦", "烦恼", "悲伤", "恐惧", "疑惑"]) :
            query_word = "人物反应"
        else:
            query_word = "gif"
        return query_word
